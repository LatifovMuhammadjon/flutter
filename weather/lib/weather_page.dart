    ...
    if (!isLoad) {
      try {
        ...
        if (response.statusCode == 200) {
          ...
          await saveBox<String?>(
              dateBox, DateFormat('dd.MM.yyyy').format(DateTime.now()));
  ...
  Future<bool> loadLocalData() async {
    try {
      ...
      if (date != null &&
        ...
        var list =
            (await getBox<List<dynamic>>(weeklyBox, defaultValue: [])) ?? [];
            ...
            children: [
              TextButton.icon(
                ...
                label: Text(
                  'Фергана',
                  style: kTextStyle(
                      color: textColor1, size: 20, fontWeight: FontWeight.w700),
                ),
              ),
              Container(
                padding:
